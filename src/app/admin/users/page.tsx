    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-300 mt-1">Kelola user dan akses platform</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white/5 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-2">Total User</h3>
          <div className="text-3xl font-bold text-blue-600">{stats.total}</div>
        </div>

        <div className="bg-white/5 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-2">User Aktif</h3>
          <div className="text-3xl font-bold text-green-400">{stats.active}</div>
        </div>

        <div className="bg-white/5 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-2">User Tidak Aktif</h3>
      ...
      <div className="bg-white/5 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-white/10">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-300 mb-2">Cari User</label>
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Cari berdasarkan email, nama, atau tenant..."
              className="w-full px-3 py-2 bg-[#0a3d47] border border-white/10 rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="px-3 py-2 bg-[#0a3d47] border border-white/10 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
            >
                      ...
                      <div className="flex-shrink-0 h-10 w-10">
                        {user.profilePicUrl ? (
                          <img
                            ...
                            alt=""
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.firstName + ' ' + user.lastName)}&background=random`;
                            }}
                          />
                        ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-2">
                      <div className="flex-shrink-0">
                        <svg className={`w-5 h-5 text-[#25D366] ${user.isWhatsAppActive ? 'animate-pulse' : ''}`} viewBox="0 0 24 24" fill="currentColor">
                      ...
                      <div className="text-sm text-cyan-300 font-bold hover:underline">
                        {user.phone ? (
                          <a
...

      {
        filteredUsers.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-400 text-lg">Tidak ada user yang ditemukan</div>
