...
export default function WhatsAppAIConfigPage() {
  ...
  const [status, setStatus] = useState<{ isConnected: boolean; phoneNumber?: string } | null>(null);
  ...
  return (
    <div className="p-3 md:p-6 max-w-5xl mx-auto">
      ...
      {status && (
        <div className={`mb-6 p-4 rounded-xl shadow-sm border-2 flex items-center justify-between ${status.isConnected ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${status.isConnected ? 'bg-green-100' : 'bg-yellow-100'}`}>
            ...
            <div>
              <p className="text-sm font-bold text-gray-900">
                {status.isConnected ? 'WhatsApp Connected' : 'WhatsApp Disconnected'}
              </p>
              <p className="text-xs text-gray-600">
                {status.isConnected ? `Number: ${status.phoneNumber}` : 'Scan QR code to reconnect'}
              </p>
            </div>
          </div>
          {status.isConnected && (
            <span className="px-2 py-0.5 bg-green-200 text-green-800 text-[10px] font-bold rounded-full">ACTIVE</span>
          )}
        </div>
      )}
