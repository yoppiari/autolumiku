  ...
  return (
    <div className="p-3 md:p-4 h-[calc(100vh-64px)] flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-4 overflow-hidden min-h-0">
        ...
        <div className={`${showChatOnMobile ? 'hidden' : 'flex'} md:flex md:col-span-4 bg-[#2a2a2a] rounded-lg shadow-sm border border-[#3a3a3a] flex-col overflow-hidden flex-1 md:flex-none`}>
          {/* Filters - Responsive */}
          <div className="p-3 md:p-3 border-b border-[#3a3a3a]">
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Cari nomor atau nama..."
              className="w-full px-3 py-2 md:py-1.5 text-sm border border-[#444] rounded-lg bg-[#333] text-white placeholder-gray-500 focus:ring-2 focus:ring-green-500 focus:border-transparent mb-2"
            />
            ...
            {filteredConversations.length === 0 ? (
              ...
              filteredConversations.map((conv) => (
                <div
                  ...
                  className={`px-4 py-3 md:px-3 md:py-2 border-b border-[#3a3a3a] cursor-pointer hover:bg-[#333] active:bg-[#333] group ${selectedConversation?.id === conv.id ? 'bg-[#333]' : ''
                    }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1 overflow-visible">
                        <div className="flex items-center gap-1">
                          <h3 className="font-medium text-white text-xs md:text-sm whitespace-nowrap">
                        ...
                        <div className="flex items-center gap-1.5 md:gap-1 mt-1 md:mt-0.5 flex-wrap">
                          ...
                          {conv.isEscalated && (
                            <span className="inline-block px-2 py-0.5 md:px-1.5 md:py-0 bg-red-900/40 text-red-300 border border-red-800/50 rounded text-[11px] md:text-[10px] font-medium">
                              Esc
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-3 md:ml-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="text-right">
                        <p className="text-[11px] md:text-[10px] text-gray-400">{formatTime(conv.lastMessageAt)}</p>
                        {conv.unreadCount > 0 && (
...
export default withRoleProtection(ConversationsPage, ROLE_LEVELS.ADMIN);
