...
export class AimeowClientService {
  ...
  static async sendImage(
    ...
    try {
      ...
      console.log(`[Aimeow Client] 📤 Sending PUBLIC URL to ${to}: ${publicUrl}`);

      // CRITICAL FIX: Aimeow API requires PUBLIC HTTP URLs, NOT base64!
      // The API will download the image from the URL we provide
      const endpoint = `${AIMEOW_BASE_URL}/api/v1/clients/${apiClientId}/send-images`;
      const payload = {
        phone: to,
        images: [{
          imageUrl: publicUrl, // ✅ PUBLIC URL - Aimeow will download it
          caption: caption || ""
        }],
        viewOnce: false,
        isViewOnce: false,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Aimeow API Error ${response.status}: ${errorText}`);
      }

      const result = await response.json();
      const msgId = result.messageId || result.id || (result.messages && result.messages[0]?.id?.id);
      console.log(`[Aimeow Client] ✅ Image sent successfully: ${msgId}`);
