...
export class WhatsAppAIChatService {
      ...
      if (aiResponse.toolCalls) {
        ...
        for (const toolCall of aiResponse.toolCalls) {
          ...
          if (toolName === 'create_lead') {
            ...
            const { to, message: replyMsg } = toolArgs;
            console.log(`[WhatsApp AI Chat] 📤 AI sending manual reply to ${to}: ${replyMsg}`);

            try {
              // Use Aimeow client directly
              const aimeowResult = await AimeowClientService.sendMessage({
              ...
              messages.push({
                role: "function",
                name: toolName,
                content: JSON.stringify({ success: aimeowResult.success, error: aimeowResult.error })
              } as any);

              if (aimeowResult.success) {
                // If we are talking to staff, confirm we sent it
                if (context.isStaff && !responseMessage) {
                  responseMessage = `Siap! Pesan sudah saya kirimkan ke ${to}. ✅`;
                }
              }
            } catch (err: any) {
              console.error(`[WhatsApp AI Chat] Failed to send manual message:`, err);
            }
  ...
  private static async handleTechnicalVehicleInquiry(
  ...
  ): Promise<{ message: string; shouldEscalate: boolean; images?: any[] } | null> {
    ...
    // PRIORITY 1: Explicit ID in current message (PM-PST-001)
    if (idInMsg) {
      contextId = idInMsg[0].toUpperCase().replace(" ", "-");
      console.log(`[SmartFallback] 🎯 Found explicit ID in message: ${contextId}`);
    }
    // PRIORITY 2: Vehicle name in current message (Daihatsu Xenia 2019)
    else if (currentMsgVehicleName && isTechnicalQuestion) {
      console.log(`[SmartFallback] 🚗 Found vehicle name in current message: ${currentMsgVehicleName}`);
      // Try to find this vehicle in database by name
      try {
        const searchName = currentMsgVehicleName.toLowerCase();
        matchingVehicle = await prisma.vehicle.findFirst({
          where: {
            tenantId,
            status: 'AVAILABLE',
            OR: [
              { make: { contains: searchName.split(' ')[0], mode: 'insensitive' as const } },
              { model: { contains: searchName.split(' ')[1] || searchName, mode: 'insensitive' as const } }
            ]
          },
          select: { id: true, displayId: true, make: true, model: true, year: true, price: true, mileage: true, transmissionType: true, color: true, variant: true, fuelType: true },
        });
        ...
        if (matchingVehicle) {
          contextId = matchingVehicle.displayId || null;
          console.log(`[SmartFallback] ✅ Matched vehicle by name: ${matchingVehicle.make} ${matchingVehicle.model} (${contextId})`);
        }
      } catch (e) {
        console.error(`[SmartFallback] Error searching vehicle by name:`, e);
      }
    }
    // PRIORITY 3: ID from conversation history (fallback only if no current context)
    else if (!contextId && isTechnicalQuestion) {
      console.log(`[SmartFallback] 🔍 No vehicle in current message, searching history...`);
      ...
      for (const aiMsg of recentAiMessages) {
        ...
        if (historyIdMatch) {
          ...
          console.log(`[SmartFallback] ⚠️ Using fallback ID from history: ${contextId}`);
          break;
        }
      }
    }

    // If we have a contextId, we try to fetch/confirm the vehicle
    if (contextId) {
      // If we don't have the vehicle yet, fetch by ID
      if (!matchingVehicle) {
        try {
          matchingVehicle = await prisma.vehicle.findFirst({
            where: {
              tenantId,
              displayId: { equals: contextId, mode: 'insensitive' as const }
            },
            select: { id: true, displayId: true, make: true, model: true, year: true, price: true, mileage: true, transmissionType: true, color: true, variant: true, fuelType: true },
          });
        } catch (e) {
          matchingVehicle = prefetchedVehicles.find(v => (v.displayId || "").toUpperCase() === contextId?.toUpperCase());
        }
      }
      ...
      if (matchingVehicle) {
        ...
        if (msg.includes("dokumen") || msg.includes("surat") || msg.includes("bpkb") || msg.includes("stnk") || msg.includes("pajak")) {
          ...
          return {
            message: `${intro}Untuk unit *${name} ${matchingVehicle.year}* (${contextId}), ${docDesc}\n\n${docClosing}`,
            shouldEscalate: false
          };
        }

        // Handle Interior/Exterior/Engine logic...
        ...
        if (hasExterior) {
          ...
          return {
            message: `${intro}${extDesc}\n\nMau asisten kirimkan **foto detail eksteriornya** untuk kelengkapan referensi? 📸`,
            shouldEscalate: false
          };
        }
        ...
        if (explicitlyAsksPhotos) {
          const images = await this.fetchVehicleImagesByQuery(contextId, context?.tenantId || tenantId);
          if (images && images.length > 0) {
            return {
              message: `Siap ${customerName}, ini foto-foto unit *${name} ${matchingVehicle.year}* (${contextId}) nya ya! MASIH READY! 🔥\n\n• Harga: Rp ${priceJuta} Juta (Nego)\n• Kondisi: Terawat, siap pakai`,
              shouldEscalate: false,
              images
            };
          }
        ...
        return {
          message: `Siap ${customerName}, unit *${name} ${matchingVehicle.year}* (${contextId}) ini MASIH READY! 🔥\n\n` +
            `• Harga: Rp ${priceJuta} Juta (Nego)\n` +
            `• Kondisi: Terawat, siap pakai\n\n` +
    ...
    // Priority 4: BRAND/MODEL SEARCH (Fallback when no explicit ID found)
    ...
    if (mentionedBrand || mentionedModel) {
      ...
      try {
        ...
        console.error(`[SmartFallback] Brand/Model search failed`, e);
      }
    }
...
`;
  ...
  private static async fetchVehicleImagesByQuery(
    ...
    if (idMatch) {
      const contextId = idMatch[0].toUpperCase().replace(" ", "-");
      console.log(`[WhatsApp AI Chat] 🎯 Explicit ID detected in query: ${contextId}`);
      ...
      console.log(`[WhatsApp AI Chat] Query result for ${contextId}:`, specificVehicle ? `FOUND (${specificVehicle.make} ${specificVehicle.model})` : 'NOT FOUND');
      ...
      if (specificVehicle) {
        if (specificVehicle.photos && specificVehicle.photos.length > 0) {
          ...
          console.log(`[WhatsApp AI Chat] ⚠️ Found specific vehicle ${contextId} but NO photos`);
          return null;
        }
      } else {
        console.log(`[WhatsApp AI Chat] ❌ Vehicle ${contextId} not found in DB (tenantId: ${tenantId})`);
